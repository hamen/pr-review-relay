...
| `--no-post` | Run every reviewer for real, print the reviews to stdout, and touch nothing on the pull request. For a PR that is not yours, where a person has to read the review before anybody else sees it. Not posting is not itself a failure, but an empty, truncated or timed-out review still exits 3: the flag changes where a review goes, not whether it counts. |
